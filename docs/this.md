...
**1.** Функция вызвана с `new`? Если да, то `this` указывает на новый сконструированный объект:

```js
const obj = new fn(); // this = какой-то новый объект
```

**2.** Функция вызвана с `call` или `apply` (явное связывание), либо `bind` (жесткое связывание)? Если да, тогда `this` указывает на явно переданный объект:
...
**3.** Функция вызвана с объектом-владельцем (неявное связывание), также называемым "контекстом"? Если да, то `this` указывает на контекстный объект:
