...
В сообщениях об ошибках в TypeScript часто фигурирует слово "assignable" (может быть назначен). В контексте множеств значений это означает либо "member of" (для отношений между значением и типом), либо "subset of" (для отношений между двумя типами).  
