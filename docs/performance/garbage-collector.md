...
По умолчанию `refCount = false` – это значит, что отписка от Источника не произойдет никогда. И, если в цепочке от Источника и выше есть удерживание ресурсов, то удерживаемый ресурс повиснет в памяти (после удаления объекта с таким `shareReplay`).

## Memory footprint

DevTools при работе активно создает объекты, которые могут увеличивать объем Memory footprint, поэтому иногда есть смысл производить замер при закрытом DevTools.
