...
Когда объект, использующий функционал `shareReplay` удаляется, то обычно подписчиков у `shareReplay` нет, но т.к. `shareReplay` с `refCount = false` не отписывается от Источника, то таймер, слушатель и т.п. не удалятся. Поэтому после удаления объекта, использующего функционал `shareReplay`, GC не сможет его убрать.
