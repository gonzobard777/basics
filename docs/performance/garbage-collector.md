...
По умолчанию `refCount = false` – это значит, что отписка от Источника не произойдет никогда. И, если в цепочке подписок есть удержание ресурса(в таймере, слушателе и т.п.), то ресурс повиснет в памяти после удаления объекта с таким `shareReplay`.
