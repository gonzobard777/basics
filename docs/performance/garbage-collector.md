...
По умолчанию `refCount = false` – это значит, что отписка от Источника не произойдет никогда. И, если в цепочке от Источника и выше есть удерживание ресурсов, то память может утечь.
