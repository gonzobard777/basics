...
По умолчанию `refCount = false` – это значит, что отписка от Источника не произойдет никогда. И, если в цепочке подписок есть удерживание ресурса, то ресурс повиснет в памяти после удаления объекта с таким `shareReplay`.
