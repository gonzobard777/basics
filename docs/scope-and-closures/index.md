...
**Блочная область видимости** – это принадлежность переменных, объявленых через `let` или `const`, произвольному блоку `{..}` Например, `if(true){..}`. Объявления же функций всегда поднимаются из произвольного блока во внешнюю область видимости.
